...
exports.resetPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      errorHandler(404, "Reset Password Token is invalid or has been expired ")
    );
  }
...
exports.getSingleUser = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      errorHandler(404, `User does not exist with Id:${req.params.id} `)
    );
  }
  res.status(200).json({ success: true, user });
});
...
exports.updateUserRole = catchAsyncErrors(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      errorHandler(400, `User does not exist with Id:${req.params.id} `)
    );
  }
  res.status(200).json({ success: true, user });
});
